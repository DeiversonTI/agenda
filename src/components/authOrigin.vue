      ...
      <div class="w-full h-auto">       
        <div class=" bg-gradient-to-l from-white via-Sky-100 to-white w-full ">          
          ...
          <div class="w-full flex justify-end items-center ">            
            <div class=" flex items-center justify-between sm:justify-end  m-1 w-full sm:w-9/12 md:w-7/12 lg:w-6/12 xl:w-5/12 2xl:w-3/12 ">
              <div>
                ...
                <form @submit.prevent="clicar(), backUp()" class="space-y-6 ">
                  ...
                  <div v-if="evExtOld === true">
                    <div>                      
                      ...
                      <div class="flex justify-between items-center sm:flex md:justify-between lg:justify-start xl:justify-start 2xl:justify-start  sm:justify-center sm:items-center space-x-1">
                        <select @change="getDataNew" name="" id="hora" class="border rounded-md w-full"  v-model="form.horariosFull" required>
                  ...
                  <div>
                      ...
                      <label class="flex opacity-70"  for="action">Local ou Situação:<p class="text-red-500 ml-1 font-extrabold">*</p> </label>
                        ...
                        <!-- <div @click="changeSelected" id="select-change" class="flex w-full text-sm flex-col flex-wrap border py-1 rounded ">
                          <select>                          
                            <option disabled></option>
                          </select>
                          <div class="overSelect"></div>
                        </div>
                        <div v-show="isOpen" id="select-links" class="select__mode">
                           <label for="salao"><input v-model="select.action" type="checkbox" value="Salão" id="salao">Salão</label> 
                           <label for="jardim"><input v-model="select.action" type="checkbox" value="Jardim Sensorial" id="jardim">Jardim Sensorial</label> 
                           <label for="agenda"><input v-model="select.action" type="checkbox" value="Agendamentos" id="agenda">Agendamentos</label> 
                           <label for="ranchinho"><input v-model="select.action" type="checkbox" value="Ranchinho" id="ranchinho">Ranchinho</label> 
                           <label for="gourmet"><input v-model="select.action" type="checkbox" value="Área Gourmet" id="gourmet">Área Gourmet</label> 
                           <label for="info"><input v-model="select.action" type="checkbox" value="Sala de Informática" id="info">Sala Informática</label> 
                           <label for="pastoral"><input v-model="select.action" type="checkbox" value="Sala Pastoral" id="pastoral">Sala Pastoral</label> 
                           <label for="labCiencias"><input v-model="select.action" type="checkbox" value="Lab. Ciências" id="labCiencias">Lab. Ciências</label> 
                           <label for="outros"><input v-model="select.action" type="checkbox" value="Outros" id="outros">Outros</label> 
                           <label for="piscina"><input v-model="select.action" type="checkbox" value="Piscina" id="piscina">Piscina</label> 
                           <label for="quadra"><input v-model="select.action" type="checkbox" value="Quadra" id="quadra">Quadra</label> 
                           <label for="rancho"><input v-model="select.action" type="checkbox" value="Rancho" id="rancho">Rancho</label> 
                           <label for="inspetor"><input v-model="select.action" type="checkbox" value="Inspetor" id="inspetor">Inspetor</label> 
                        </div>-->
                      <!-- </div> -->
                      <!-- <div>
                        <div class="" >
                          <ul class="flex flex-wrap" >
                            <li v-for="(sel, index) in select.action" :key="index" class="rounded py-0 px-2 mt-1 mr-1 bg-Sky-500 text-gray-50">{{ sel }} </li>                            
                          </ul>
                        </div>
                      </div> -->    
                  </div>                  
...
export default {
    ...
    if 
    (
      this.form.dataNew === getFull && this.form.horariosFull === hFull &&  this.form.situacao === sitUser
    ...
    //LAÇO DE REPETIÇAO PARA BLOQUEAR TODO O SEGUIMENTO, CASO ALGUÉM USO A DATA FULL        
      ...
      this.form.dataNew === getFull && hFull === '07h10-12h' && this.form.situacao === sitUser
      ) 
      {
      if(
...
@import url('https://fonts.googleapis.com/css2?family=Poppins:ital,wght@0,300;0,500;0,800;1,100&family=Prompt:ital,wght@0,100;0,200;0,400;0,700;0,800;1,500;1,900&display=swap');
  ...
  .select__mode  label:hover{
   background-color: #52a9ff;
   color:#fff
  }
  .select__mode  label input{
    margin: 0 5px;
    display: none;    
  }
  .select__mode  label:checked {
